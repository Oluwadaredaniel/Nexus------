...
import { Building2, Plus, Layers, Split, Trash2 } from 'lucide-react';
...
export default function ManageFaculties() {
  ...
  const onDeleteFaculty = async (id: string) => {
    if(!window.confirm("Delete this Faculty? All departments inside it will be lost.")) return;
    try {
  ...
  const onDeleteDept = async (facultyId: string, deptId: string) => {
    if(!window.confirm("Delete this Department?")) return;
    try {
      await api.delete(`/admin/faculties/${facultyId}/departments/${deptId}`);
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">Manage academic hierarchy (Faculty {'>'} Department {'>'} Options).</p>
        </div>
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {faculties.map((faculty, i) => (
          ...
          >
            <Card className="h-full glass-card border-white/5 hover:border-primary/20 flex flex-col">
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                ...
                <div className="flex gap-1">
                  <Button variant="ghost" size="icon" className="h-8 w-8 hover:text-red-400" onClick={() => onDeleteFaculty(faculty._id)}>
                    <Trash2 className="h-4 w-4" />
                  </Button>
                  <Button variant="ghost" size="sm" className="h-8" onClick={() => openDeptModal(faculty._id)}>
                ...
                <div className="mt-2 space-y-3">
                  ...
                  <ul className="space-y-2">
                    {faculty.departments?.map((dept: any) => (
                      <li key={dept._id} className="text-sm p-3 rounded-lg bg-white/5 space-y-2 group relative">
                         <div className="flex items-center justify-between font-medium">
                           <div className="flex items-center gap-2">
                             <Layers className="h-4 w-4 text-muted-foreground" />
                             {dept.name}
                           </div>
                           <button className="opacity-0 group-hover:opacity-100 text-red-400 transition-opacity" onClick={() => onDeleteDept(faculty._id, dept._id)}>
        ...
        <form onSubmit={subDept(onAddDept)} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <textarea
              ...
              rows={3}
            />
            <p className="text-xs text-muted-foreground">Split sub-tracks with commas.</p>
          </div>
