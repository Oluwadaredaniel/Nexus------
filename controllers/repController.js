...
export const createSession = async (req, res) => {
  ...
  try {
    ...
    // Emit socket event (handled in index.js via function call or event bus if separated, here assuming direct db response is enough for frontend to trigger socket join)
...
export const extendSession = async (req, res) => {
  ...
  try {
    ...
    session.isActive = true; // Re-activate if closed
    await session.save();
    res.json(session);
  } catch (error) { res.status(400).json({ message: error.message }); }
};
