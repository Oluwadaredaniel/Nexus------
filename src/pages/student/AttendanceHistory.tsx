...
export default function AttendanceHistory() {
  ...
  if (loading) return <div className="p-10 text-center animate-pulse">Loading records...</div>;
              ...
              <tbody className="divide-y divide-white/5">
                {history.map((record, i) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <div className="font-semibold text-white">{record.session.course.code}</div>
                      <div className="text-xs text-muted-foreground">{record.session.course.title}</div>
                    </td>
                    <td className="px-6 py-4 flex items-center gap-2 text-muted-foreground">
                      <CalendarDays className="h-4 w-4" />
                      {formatDate(record.session.startTime).split(',')[0]}
                    </td>
